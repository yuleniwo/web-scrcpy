class ScrcpyInput {
	constructor(callback, videoElement, width, height, debug = false) {
		...
		document.addEventListener('mouseup', (event) => {
			if (!leftButtonIsPressed && !rightButtonIsPressed) return;
		...
		document.addEventListener('touchstart', (event) => {
			const rect = videoElement.getBoundingClientRect();
			const local_x = event.touches[0].clientX - rect.left;
			const local_y = event.touches[0].clientY - rect.top;

			if (videoElement.contains(event.target)) {
				event.preventDefault();
				leftButtonIsPressed = true;

				mouseX = Math.round((local_x / (rect.right - rect.left)) * this.width);
				mouseY = Math.round((local_y / (rect.bottom - rect.top)) * this.height);
				//console.log('touchStart x:' + mouseX + ', y:' + mouseY + ', w:' + this.width + ', h:' + this.height);
				let data = this.createTouchProtocolData(0, mouseX, mouseY, this.width, this.height, 0, 0, 65535);
				this.callback(data);
			}
		}, { passive: false }); // 注意：如果使用了 preventDefault，必须设置 passive: false
		
		document.addEventListener('touchend', (event) => {
			if (!leftButtonIsPressed) return;
			const th = event.changedTouches.length > 0 ? event.changedTouches[0] : event.touches[0];
			
			const rect = videoElement.getBoundingClientRect();
			const local_x = th.clientX - rect.left;
			const local_y = th.clientY - rect.top;
			...
			if (videoElement.contains(event.target)) {
				...
				mouseY = Math.round((local_y / (rect.bottom - rect.top)) * this.height);
			//}
			//console.log('touchEnd x:' + mouseX + ', y:' + mouseY + ', w:' + this.width + ', h:' + this.height);
			let data = this.createTouchProtocolData(1, mouseX, mouseY, this.width, this.height, 0, 0, 0);
			this.callback(data);}
		});
		...
		videoElement.addEventListener('wheel', (event) => {
			event.preventDefault();
			const hScroll = event.deltaX > 0 ? -32768 : event.deltaX < -0 ? 32767 : 0;
            const vScroll = event.deltaY > 0 ? -32768 : event.deltaY < -0 ? 32767 : 0;
			...
			const vw = rect.right - rect.left;
			const vh = rect.bottom - rect.top;
			const x = Math.round(vx / vw * this.width);
			const y = Math.round(vy / vh * this.height);
			
			let data = this.createScrollProtocolData(x, y, this.width, this.height, hScroll, vScroll, buttons);
			this.callback(data);
		});
	...
	sendKeyCode(keyevent, action, keycode) {
		let capsLockState = 0, numLockState = 0, scrollLockState = 0;
		if(keyevent.getModifierState != null) {
			capsLockState = Number(keyevent.getModifierState('CapsLock'));
			numLockState = Number(keyevent.getModifierState('NumLock'));
			scrollLockState = Number(keyevent.getModifierState('ScrollLock'));
		}
		const repeat = keyevent.repeat != null ? Number(keyevent.repeat) : 0;
	...
	createScrollProtocolData(x, y, width, height, hScroll, vScroll, buttons) {
		const type = 3; // SC_CONTROL_MSG_TYPE_INJECT_SCROLL_EVENT
	...
	// 显示或关闭屏幕（不影响投屏）
	sendSetDisplayPower(action) { // 0:off; 1:on
		const type = 10; // app/src/control_msg.h -> SC_CONTROL_MSG_TYPE_SET_DISPLAY_POWER
	...
	// 屏幕已解锁状态下会产生Back消息，否则会亮屏等待解锁。
	sendBackOrScreenOn(action) {
		const type = 4; // SC_CONTROL_MSG_TYPE_BACK_OR_SCREEN_ON
		const buffer = new ArrayBuffer(2);
		const view = new DataView(buffer);
		view.setUint8(0, type);
		view.setUint8(1, action);
	...
	sendResetVideo() {
		const type = 17; // SC_CONTROL_MSG_TYPE_RESET_VIDEO
